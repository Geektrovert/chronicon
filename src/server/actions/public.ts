...
export const readPublicDocument = Effect.fn("Public.document")(function* (
  ...
  const telemetry = { document_id: found.id };

  if (current.publicProjectId) Object.assign(telemetry, { project_id: current.publicProjectId });
  yield* annotateTelemetry(telemetry);
